...
void Socket::setLocalPort(unsigned short localPort, bool bindPort) throw(SocketException) {
	...
	if (bindPort){
		int ret = bind(sockDesc, (sockaddr *)&localAddr, sizeof(sockaddr_in));
		if (ret == SOCKET_ERROR) {
#if IBM
			...
			int error = errno;
#endif
			cout << "Set of local port failed (bind()) : returs " << error << "\n";
		}
	}
}
...
void Socket::setLocalAddressAndPort(const string &localAddress,
	...
	setLocalAddressAndPort(localAddress, localPort, true);
}

void Socket::setLocalAddressAndPort(const string &localAddress,
	unsigned short localPort, bool isBind) throw(SocketException) {
	// Get the address of the requested host
	sockaddr_in localAddr;
	fillAddr(localAddress, localPort, localAddr);
	if (isBind) {
		int res = bind(sockDesc, (sockaddr *)&localAddr, sizeof(sockaddr_in));
		if (res < 0) {
...
void Socket::setReuse() {
	int option = 1;
	setsockopt(sockDesc, SOL_SOCKET, SO_REUSEADDR, (raw_type *) &option, sizeof(option));
}
...
int CommunicatingSocket::send(const char *buffer, int bufferLen)  throw(SocketException) {
	int ret = ::send(sockDesc, buffer, bufferLen, 0);
	if (ret < 0) {
...
int CommunicatingSocket::recv(char *buffer, int bufferLen) throw(SocketException) {
	int ret	= ::recv(sockDesc, buffer, bufferLen, 0);
	if (ret < 1){
...
UDPSocket::UDPSocket(unsigned short localPort)  throw(SocketException) : 
	...
	setLocalPort(localPort, false);
	setBroadcast();
	setReuse();
}

UDPSocket::UDPSocket(const string &localAddress, unsigned short localPort, bool isBind)
throw(SocketException) : CommunicatingSocket(SOCK_DGRAM, IPPROTO_UDP) {
	setLocalAddressAndPort(localAddress, localPort, isBind);
	setBroadcast();
	setReuse();
}
...
void UDPSocket::setBroadcast() {
	...
	int option = 1;
	setsockopt(sockDesc, SOL_SOCKET, SO_BROADCAST, (raw_type *) &option, sizeof(option));
	setsockopt(sockDesc, SOL_SOCKET, SO_REUSEADDR, (raw_type *) &option, sizeof(option));
}
...
throw(SocketException) {
	...
	ret = sendto(sockDesc, buffer, bufferLen, 0, (sockaddr *) &destAddr, sizeof(destAddr));
	// Write out the whole buffer as a single message.
	if (ret == SOCKET_ERROR/*ret  != bufferLen*/) {
#if IBM
		int error = WSAGetLastError();
#else
		int error = errno;
#endif
		char s[1024];
		sprintf(s, "Send failed (sendto())%d", error);
		throw SocketException(s, true);
	}
	return ret;
}

int UDPSocket::recvFrom(char *buffer, int bufferLen, string &sourceAddress,
		unsigned short &sourcePort) throw(SocketException) {
	sockaddr_in clntAddr;
	socklen_t addrLen = sizeof(clntAddr);
	int rtn;
	if ((rtn = recvfrom(sockDesc, buffer, bufferLen, 0,
...
#if IBM
	multicastRequest.imr_multiaddr.S_un.S_addr = inet_addr(multicastGroup.c_str());
	multicastRequest.imr_interface.S_un.S_addr = htonl(INADDR_ANY);
#else
